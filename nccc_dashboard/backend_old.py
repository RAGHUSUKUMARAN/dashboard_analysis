...
# 🔥 NORMALIZE EXCLUSION LIST (NOW SAFE)
for key in EXCLUDED_IPS:
    new_set = set()

    for ip in EXCLUDED_IPS[key]:
        if "/" in ip:
            val = normalize_cidr(ip)
            if not val:
                log(f"Invalid CIDR skipped: {ip}")
                continue
        else:
            val = normalize_ip(ip)
...
def build_branch_ip_files(ioc_dir, output_dir):
    ...
    for f in ioc_dir.rglob("*_ioc_hits_mail.csv"):
        ...
        ips = set(df["Source IP"].dropna()) | set(df["Dstn IP"].dropna())

        if branch not in branch_ip_map:
            branch_ip_map[branch] = set()

        # 🔥 APPLY EXCLUSION (if config exists)
        ...
        excluded_count = len(ips) - len(filtered_ips)
        log(f"{branch}: {len(ips)} → {len(filtered_ips)} (excluded {excluded_count})")

        branch_ip_map[branch].update(filtered_ips)

    if not branch_ip_map:
        log("No branch IP data found")
        return

    # 🔥 WRITE FILE PER BRANCH
    for branch, ips in branch_ip_map.items():
        ...
        log(f"{branch} → {len(ip_list)} IPs saved at {out_file}")
...
def process(folder):
    ...
    try:
        ...
        # Executive PDF saved as SOC_REPORT_EXEC.pdf
