...
def page_header(title):
    """Inline header inside page_content — kept for internal use."""
    return html.Div([
        html.Div("◆", style={"color": "#00ffff", "marginRight": "8px"}),
...
def breadcrumb_bar():
    """
    Persistent strip shown between top_bar and page_content.
    Hidden on home (/). Updates via callback on route change.
    Contains: ⌂ Home › Current Page label + a glowing HOME button.
    """
    return html.Div(
        id="breadcrumb_bar",
        style={"display": "none"},   # callback shows/hides
    )
...
def reports_page():
    return html.Div([
        page_header("DCyA Reports for SOC"),
...
def mail_page():
    return html.Div([
        page_header("IOC Hit Mail Files"),
...
def unique_ips_page():
    return html.Div([
        page_header("Unique IP Inventory (Branch-wise)"),

        html.Iframe(
            src="/ip_inventory_browser",
            style={
                "width": "100%",
                "height": "75vh",
                "border": "none",
...
def top_bar():
    return html.Div([
        # ── Left: Logo + Home button (home btn hidden on home page) ──
        html.Div([
            html.Span("◈ NCCC // TRAFFIC MONITORING & ANALYSIS",
                      style=mono(18, ACCENT, "bold")),

            # 🔥 Home button — shown only on sub-pages (callback controls display)
            html.A(
                [
                    html.Span("⌂", style={"fontSize": "13px", "marginRight": "5px"}),
                    html.Span("HOME", style={"fontSize": "10px", "letterSpacing": "1.5px"}),
                ],
                id="home_btn",
                href="/",
                style={
                    "display": "none",          # hidden by default; shown on sub-pages
                    "alignItems": "center",
                    "textDecoration": "none",
        ...
        html.Div([
            ...
            # 🔥 HOVER EFFECT (Dash trick via inline JS-style props)
...
def nav_drawer():
    ...
    return html.Div([
        ...
        html.Div([
            ...
            nav_link("🌐", "Unique IP Inventory", "/ip_inventory", "Branch-wise unique IPs"),
...
def serve_layout():
    return html.Div([
        ...
        # Page container — content swapped by route callback
