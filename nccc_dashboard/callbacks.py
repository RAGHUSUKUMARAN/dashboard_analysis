...
# ─────────────────────────────────────────────────────────────
# Hamburger button base style (shared between layout & callback)
# ─────────────────────────────────────────────────────────────
HAMBURGER_STYLE = {
    ...
    "boxShadow": "0 0 8px rgba(0,255,255,0.15), inset 0 0 6px rgba(0,255,255,0.05)",
    "zIndex": "2000",
    "marginLeft": "10px",
}

# ─────────────────────────────────────────────────────────────
# Page metadata — route → (label, icon)
# ─────────────────────────────────────────────────────────────
PAGE_META = {
    "/charts":       ("Charts View",          "◈"),
    "/reports":      ("DCyA Reports for SOC", "📋"),
    "/json":         ("JSON Files",           "{ }"),
    "/csv":          ("Cleaned CSV Files",    "⊞"),
    "/mail":         ("IOC Hit Mail Files",   "✉"),
    "/ip_inventory": ("Unique IP Inventory",  "🌐"),
}
...
def register_callbacks(app):
    ...
    def route_pages(pathname):
        if pathname == "/charts":
            return charts_page()
        elif pathname == "/ip_inventory":
            return unique_ips_page()
    ...
    # ================= BREADCRUMB BAR =================
    @app.callback(
        Output("breadcrumb_bar", "children"),
        Output("breadcrumb_bar", "style"),
        Output("home_btn",       "style"),
        Input("url", "pathname"),
    )
    def update_breadcrumb(pathname):
        """
        Shows a slim breadcrumb strip on sub-pages.
        Also shows/hides the ⌂ HOME button in the top bar.
        """
        ...
        home_btn_hidden = {**home_btn_visible, "display": "none"}

        if is_home:
            return [], {"display": "none"}, home_btn_hidden

        # ── Build breadcrumb for sub-page ─────────────────────────────
        icon, label = PAGE_META.get(pathname, ("◆", pathname.strip("/").upper()))
        if isinstance(icon, str) and len(PAGE_META.get(pathname, ())) == 2:
            icon, label = PAGE_META[pathname]
        ...
        sep = html.Span("›", style={"color": MUTED, "fontSize": "13px"})

        crumb_children = [
            # ⌂ Home link
            html.A(
                [html.Span("⌂", style={"marginRight": "4px"}), "HOME"],
                href="/",
                style={
                    "color":          MUTED,
                    "textDecoration": "none",
            ...
            # Current page
            html.Span(
                [html.Span(icon, style={"marginRight": "5px"}), label],
                style={
                    "color":         ACCENT,
                    "fontWeight":    "bold",
                    "letterSpacing": "0.8px",
        ...
        return crumb_children, crumb_style, home_btn_visible


    # ================= NAV DRAWER TOGGLE =================
    # Single callback owns all three outputs.
    # Triggers: hamburger open | X close | overlay click | URL change (auto-close)
    _DRAWER_BASE = {
        "position":   "fixed",
        "top":        "0",
        "right":      "-320px",          # default: hidden off-screen
        "width":      "300px",
        "height":     "100vh",
        "background": CARD,
    ...
    def toggle_nav_drawer(open_clicks, close_clicks, overlay_clicks, pathname):
        ...
        if triggered == "nav_toggle":
            # OPEN — slide drawer in, hide hamburger, show overlay
            return (
            ...
            # CLOSE — slide drawer out, show hamburger, hide overlay
            return (
                {**_DRAWER_BASE, "right": "-320px"},
                {**HAMBURGER_STYLE, "display": "flex"},
...
def register_pdf_route(flask_app):
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    <!-- 🔥 HEADER -->
    <div class="header">
      <div class="header-title">◈ NCCC // TRAFFIC MONITORING & ANALYSIS</div>
    ...
    {% if not files %}
    <div class="empty">
      ⊘ No files found{% if not base_path or not folder_exists %} — run pipeline first{% endif %}
    </div>
    ...
    def browse_reports():
        ...
        if not folder:
            return render_template_string(_BROWSER_TMPL, title="DCyA Reports", files=[], base_path="", folder_exists=False)

        base = Path(folder)

        # 🔥 FIX: handle if user gives input folder instead of root
        dcya = None

        candidates = [
            base / "output" / "reports" / "dcya",
            base.parent / "output" / "reports" / "dcya"
        ]
        ...
        print("DCYA PATH:", dcya)

        files, exists, bpath = _build_file_list(
            dcya,
            None,   # 🔥 no filter
            "/serve/reports"
        )
    ...
    def browse_json():
        ...
        candidates = [
            base_root / "output" / "reports" / "json",
            base_root.parent / "output" / "reports" / "json"
        ]
    ...
    def browse_csv():
        ...
        candidates = [
            base_root / "output" / "csv_cleaned",
            base_root.parent / "output" / "csv_cleaned",
        ]
    ...
    def browse_mail():
        ...
        candidates = [
            base_root / "output" / "ioc_hits_mail",
            base_root.parent / "output" / "ioc_hits_mail",
        ]
    ...
    # ── /unique ip →  output/reports/ip_inventory ────────────────────────────────────
    @flask_app.route("/ip_inventory_browser")
    def browse_ip_inventory():
        folder = progress.get("folder")

        if not folder:
            return render_template_string(
                _BROWSER_TMPL,
                title="Unique IP Inventory",
                files=[],
                base_path="",
                folder_exists=False,
            )
        ...
        candidates = [
            base_root / "output" / "reports" / "ip_inventory",
            base_root.parent / "output" / "reports" / "ip_inventory"
        ]
        ...
        files, exists, bpath = _build_file_list(base, {".csv"}, "/serve/ip_inventory")
    ...
    flask_app.add_url_rule(
    "/serve/ip_inventory/<path:filepath>",
    view_func=_make_server(
        lambda f: Path(f) / "output" / "reports" / "ip_inventory", "ip_inventory"
    )
)
