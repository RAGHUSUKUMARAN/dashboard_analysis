...
def header_footer(canvas, doc):
    ...
    canvas.drawCentredString(w/2, 3.5*mm, "National Cyber Coordination Centre")
...
def build_cover(canvas, doc, summary, ts):
    ...
    canvas.drawString(MARGIN, h - 7*mm, "National Cyber Coordination Centre")

    # Large NCCC acronym centred in header
    canvas.setFont("Helvetica-Bold", 38)
    canvas.setFillColor(WHITE)
    canvas.drawCentredString(w / 2, h - header_h + 14*mm, "JCOCC")
    ...
    canvas.setFillColor(DARK_NAVY)
    canvas.drawString(MARGIN + 5*mm, h * 0.67, "NCCC TRAFFIC ANALYSIS")
    ...
    canvas.drawString(MARGIN + 5*mm, h * 0.565,
                      "JCOCC Lab ·  Traffic Monitoring & Analysis Summary")
    ...
    canvas.setFont("Helvetica", 7)
    canvas.setFillColor(colors.HexColor("#A8DADC"))
    canvas.drawString(MARGIN, 5.5*mm, "JCOCC - DCyA  ·  Traffic Monitoring Branch")
